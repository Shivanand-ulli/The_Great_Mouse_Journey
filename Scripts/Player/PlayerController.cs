...
public class PlayerController : MonoBehaviour
{
    ...
    void HandleEmotes()
    {
        if (Input.GetKeyDown(KeyCode.E)) // Press 'E' for waving
        {
            PlayEmote("Wave");
        }
        if (Input.GetKeyDown(KeyCode.T)) // Press 'T' for waving
        {
            PlayEmote("Salsa");
        }
    }
    ...
    void HandleJump()
    {
        ...
        {
            ySpeed = -0.5f;
            SetAnimationState(true, false, false); // Grounded, not jumping, not falling

            if (Time.time - jumpButtonPressedTime <= jumpButtonGracePeriod)
            {
                ySpeed = Mathf.Sqrt(jumpHeight * -2 * Physics.gravity.y * gravityMultiplier);
                isJumping = true;
                SetAnimationState(false, true, false); // Not grounded, jumping, not falling
